...
class Pitch:
    def __init__(self, title="", half=None, width=680, brand=True, subtitle="", attack_arrow=True):
        ...
        self.defs = []
        self.body = []
        self.top = []                 # capa superior: jugadores, balón y notas (encima de flechas)
        self._cur = self.body
        self._legend = None
        self._draw_pitch()
    ...
    def _rect(self, x, y, w, h, **kw):
        ...
        self._cur.append(f'<rect x="{x:.1f}" y="{y:.1f}" width="{w:.1f}" height="{h:.1f}" {a}/>')
    def _line(self, x1, y1, x2, y2, w=2.2, c=None, dash=None, opacity=1):
        c = c or PALETTE["line"]
        d = f' stroke-dasharray="{dash}"' if dash else ""
        self._cur.append(f'<line x1="{x1:.1f}" y1="{y1:.1f}" x2="{x2:.1f}" y2="{y2:.1f}" '
                         f'stroke="{c}" stroke-width="{w}"{d} opacity="{opacity}"/>')
    def _circle(self, cx, cy, r, **kw):
        a = " ".join(f'{k.replace("_","-")}="{v}"' for k, v in kw.items())
        self._cur.append(f'<circle cx="{cx:.1f}" cy="{cy:.1f}" r="{r:.1f}" {a}/>')
    def _text(self, x, y, s, size=12, c=None, w=700, anchor="middle", style=""):
        c = c or PALETTE["text"]
        self._cur.append(f'<text x="{x:.1f}" y="{y:.1f}" font-family="Segoe UI,Arial,sans-serif" '
    ...
    def _draw_pitch(self):
        ...
        if self.brand:
            ...
            self._text(12, self.H - 8, "Balón parado defensivo · MISTER ÉLITE", size=10, c="#7f93a8", w=600, anchor="start")
    ...
    def _penalty_area(self, top, lw):
        ...
        if top:
            ...
            spoty = self.Y(11)
        # gran área
        self._cur.append(f'<path d="M{xL:.1f},{yb:.1f} L{xL:.1f},{yfront:.1f} L{xR:.1f},{yfront:.1f} '
                         f'L{xR:.1f},{yb:.1f}" fill="none" stroke="{PALETTE["line"]}" stroke-width="{lw}"/>')
        # área pequeña
        self._cur.append(f'<path d="M{gxL:.1f},{yb:.1f} L{gxL:.1f},{ygf:.1f} L{gxR:.1f},{ygf:.1f} '
        ...
        self._cur.append(f'<rect x="{gxgL:.1f}" y="{min(gy,yb):.1f}" width="{(gxgR-gxgL):.1f}" '
        ...
        import math
        if top:
            self._cur.append(f'<path d="M{cx-self.S(7):.1f},{yfront:.1f} A{r:.1f},{r:.1f} 0 0 1 '
                             f'{cx+self.S(7):.1f},{yfront:.1f}" fill="none" stroke="{PALETTE["line"]}" stroke-width="{lw}"/>')
        else:
            self._cur.append(f'<path d="M{cx-self.S(7):.1f},{yfront:.1f} A{r:.1f},{r:.1f} 0 0 0 '
                             f'{cx+self.S(7):.1f},{yfront:.1f}" fill="none" stroke="{PALETTE["line"]}" stroke-width="{lw}"/>')
    ...
    def _attack_arrow(self):
        ...
        self._cur.append(f'<g opacity="0.9"><line x1="{x:.1f}" y1="{y1:.1f}" x2="{x:.1f}" y2="{y2:.1f}" '
    ...
    def player(self, x, y, label, team="own", role="", r=15, number=None, role_below=False):
        self._cur = self.top                      # los jugadores van por encima de las flechas
        ...
        self._cur.append(f'<g>{ring}<circle cx="{cx:.1f}" cy="{cy:.1f}" r="{r}" fill="{fill}" '
    ...
    def ball(self, x, y, r=6):
        ...
        self._cur.append(f'<path d="M{cx:.1f},{cy-r:.1f} l{r*0.5:.1f},{r*0.7:.1f} l-{r*0.8:.1f},0 z" '
    ...
    def arrow(self, x1, y1, x2, y2, kind="pass", c=None, label=""):
        ...
        if kind == "dribble":
            ...
            self._cur.append(f'<path d="{path}" fill="none" stroke="{col}" stroke-width="2.6" '
                             f'marker-end="url(#{mk})"/>')
        else:
            self._cur.append(f'<line x1="{X1:.1f}" y1="{Y1:.1f}" x2="{X2:.1f}" y2="{Y2:.1f}" '
    ...
    def zone(self, x1, y1, x2, y2, label="", c=None, fill_op=0.16, dash="6 5", ellipse=False):
        ...
        if ellipse:
            ...
            self._cur.append(f'<ellipse cx="{cx:.1f}" cy="{cy:.1f}" rx="{abs(X2-X1)/2:.1f}" '
                             f'ry="{abs(Y2-Y1)/2:.1f}" fill="{c}" fill-opacity="{fill_op}" '
                             f'stroke="{c}" stroke-width="2" stroke-dasharray="{dash}"/>')
        else:
            self._cur.append(f'<rect x="{X1:.1f}" y="{Y1:.1f}" width="{abs(X2-X1):.1f}" '
    ...
    def cone(self, x, y):
        cx, cy = self.X(x), self.Y(y)
        self._cur.append(f'<path d="M{cx:.1f},{cy-7:.1f} l5,11 l-10,0 z" fill="#ff9800" stroke="#7a4f00" stroke-width="1"/>')

    def goalmini(self, x, y):  # mini portería para tareas
        cx, cy = self.X(x), self.Y(y)
        self._cur.append(f'<rect x="{cx-12:.1f}" y="{cy-3:.1f}" width="24" height="6" fill="none" '
    ...
    def svg(self):
        return (f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {self.W:.0f} {self.H:.0f}" '
                ...
                + self._markers() + "".join(self.body) + "".join(self.top)
