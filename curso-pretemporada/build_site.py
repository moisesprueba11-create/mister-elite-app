#!/usr/bin/env python3
"""Construye una versión HTML navegable del curso de Pretemporada a partir de los .md.
Salida: curso-pretemporada/site/  (abrir site/index.html)."""
...
PAGES = [
    ...
    ("ejercicios/03-plan-5-sesiones.md", "ejercicios/03-plan-5-sesiones.html", "Plan · 5 sesiones", "Planes"),
    ("ejercicios/04-plan-4-sesiones.md", "ejercicios/04-plan-4-sesiones.html", "Plan · 4 sesiones", "Planes"),
...
def build_nav(current_out, prefix):
    ...
    parts.append(f'<a class="brand" href="{prefix}index.html">📋 Pretemporada perfecta</a>')
    for sec in order:
...
<title>{title} · Pretemporada perfecta</title>
    ...
    for src, out, title, sec in PAGES:
        ...
        if out == "index.html":
            body = (f'<img class="hero" src="{prefix}graficos/portada.svg" '
                    f'alt="Pretemporada perfecta · MISTER ÉLITE">' + body)
